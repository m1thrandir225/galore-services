...
func createRandomUser(t *testing.T) CreateUserRow {
	hashedPassword, err := security.HashPassword("lol1231231231")
	require.NoError(t, err)

	dbDate, err := util2.TimeToDbDate(util2.RandomDate())

	require.NoError(t, err)

	name := util2.RandomString(12)
...
func TestUpdateUserInformation(t *testing.T) {
	user := createRandomUser(t)
	newBirthday, err := util2.TimeToDbDate(util2.RandomDate())

	arg := UpdateUserInformationParams{
		ID:        user.ID,
		Email:     util2.RandomEmail(),
		AvatarUrl: util2.RandomString(46),
		Name:      util2.RandomString(36),
...
func TestUpdateUserPassword(t *testing.T) {
	...
	newPassword, err := security.HashPassword(util2.RandomString(32))
	require.NoError(t, err)
